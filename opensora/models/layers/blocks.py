    ...
    def forward(self, x: torch.Tensor, kv_cache = None) -> torch.Tensor:
        if kv_cache is None:
            ...
            if self.qk_norm_legacy:
                ...
                q, k = self.q_norm(q), self.k_norm(k)
                if self.rope:
                    q = self.rotary_emb(q, offset=7)
                    k = self.rotary_emb(k, offset=7)
...
class T2IFinalLayer(nn.Module):
    ...
    def forward(self, x, t, x_mask=None, t0=None, T=None, S=None):
        ...
        if S is None:
            S = self.d_s
        t = self.scale_shift_table[None] + t[:, None]
        t = rearrange(t, "B C d -> B 1 C d")
        shift, scale = t.chunk(2, dim=2)
        x = t2i_modulate(self.norm_final(x), shift, scale)
        if x_mask is not None:
            t0 = self.scale_shift_table[None] + t0[:, None]
            t0 = rearrange(t0, "B C d -> B 1 C d")
            shift_zero, scale_zero = t0.chunk(2, dim=2)
        ...
        #     t0 = self.scale_shift_table[None] + t0[:, None]
        #     # t0 = rearrange(t0, "B C d -> B 1 C d")
        #     shift_zero, scale_zero = t0.chunk(2, dim=1)
        #     x_zero = t2i_modulate(self.norm_final(x), shift_zero, scale_zero)
        #     x = self.t_mask_select(x_mask, x, x_zero, T, S)
        #     # x = torch.where(x_mask[:, :, None, None], x, x_zero)
        # x = self.linear(x)
