...
class ActionEncoderWithLearnedPosition(nn.Module):
    def __init__(self, hidden_size):
        ...
        """
        super(ActionEncoderWithLearnedPosition, self).__init__()
        
        # 7个输入，每个输入范围是0-256
        self.embedding_layers = nn.ModuleList(
            [nn.Embedding(257, hidden_size) for _ in range(7)]
        )
        
        # 学习型位置编码
        self.position_embedding = nn.Embedding(7, hidden_size)
        
        # 全连接层
        self.fc = nn.Sequential(
            nn.Linear(7 * hidden_size, hidden_size*4),
            nn.SiLU(),
    ...
    def forward(self, x):
        """
        前向传播
        :param x: 输入形状为 (batch_size, 7)，每列是0-255之间的整数
        :return: 编码后的动作向量 (batch_size, output_dim)
        """
        batch_size = x.size(0)
        
        # 对每个整数通过embedding生成嵌入
        embedded = [layer(x[:, i]) for i, layer in enumerate(self.embedding_layers)]
        embedded_concat = torch.stack(embedded, dim=1)  # (batch_size, 7, embedding_dim)
        
        # 添加学习型位置编码
        positions = torch.arange(7).unsqueeze(0).expand(batch_size, -1).to(x.device)
        position_embeds = self.position_embedding(positions)  # (batch_size, 7, embedding_dim)
        
        # 将位置编码加到嵌入
        embedded_concat += position_embeds
        
        # Flatten嵌入 (batch_size, 7 * hidden_size)
        embedded_concat = embedded_concat.view(batch_size, -1)
        
        # 全连接层
        output = self.fc(embedded_concat)
        return output


class CacheSTDiT3Block(nn.Module):
    def __init__(
        self,
        hidden_size,
        num_heads,
    ...
    def forward(
        ...
        is_kv_cache_none = kv_cache is None
        ...
        if self.temporal:
            ...
            if kv_cache is None:
                x_m, kv_cache = self.attn(x_m)
            else:
                x_m = self.attn(x_m, kv_cache)
        ...
        if self.temporal and is_kv_cache_none:
            return x,kv_cache
        else:
            return x


class CacheSTDiT3Config(PretrainedConfig):
    model_type = "CacheSTDiT3"
...
class CacheSTDiT3(PreTrainedModel):
    config_class = CacheSTDiT3Config
    ...
    def __init__(self, config):
        ...
        self.action_embedder = ActionEncoderWithLearnedPosition(config.hidden_size)
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None, fps=None, height=None, width=None, kv_caches = None,**kwargs):
        is_kv_caches_none = kv_caches is None
        ...
        action_embedding = self.action_embedder(y)
        ...
        # === blocks ===
            for spatial_block, temporal_block in zip(self.spatial_blocks, self.temporal_blocks):
                ...
                x, kv_cache = auto_grad_checkpoint(temporal_block, x, t_mlp, x_mask, t0_mlp, T, S)
                kv_caches.append(kv_cache)
        else:
            for spatial_block, temporal_block, kv_cache in zip(self.spatial_blocks, self.temporal_blocks, kv_caches):
                x = auto_grad_checkpoint(spatial_block, x, t_mlp, x_mask, t0_mlp, T, S)
                x = auto_grad_checkpoint(temporal_block, x, t_mlp, x_mask, t0_mlp, T, S, kv_cache)
...
@MODELS.register_module("CacheSTDiT3-XL/2")
def CacheSTDiT3_XL_2(from_pretrained=None, **kwargs):
    ...
    if force_huggingface or from_pretrained is not None and not os.path.exists(from_pretrained):
        model = CacheSTDiT3.from_pretrained(from_pretrained, **kwargs)
    else:
        config = CacheSTDiT3Config(depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
        # 'qk_norm':
        # True
        # 'enable_flash_attn':
        # True
        ...
        model = CacheSTDiT3(config)
        if from_pretrained is not None:
            load_checkpoint(model, from_pretrained)
    return model


@MODELS.register_module("CacheSTDiT3-3B/2")
def CacheSTDiT3_3B_2(from_pretrained=None, **kwargs):
    force_huggingface = kwargs.pop("force_huggingface", False)
    if force_huggingface or from_pretrained is not None and not os.path.exists(from_pretrained):
        model = CacheSTDiT3.from_pretrained(from_pretrained, **kwargs)
    else:
        config = CacheSTDiT3Config(depth=28, hidden_size=1872, patch_size=(1, 2, 2), num_heads=26, **kwargs)
        model = CacheSTDiT3(config)
        if from_pretrained is not None:
            load_checkpoint(model, from_pretrained)
    return model
